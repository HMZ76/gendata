...
def convnext_3d_tiny(pretrained=False, in_22k=False, **kwargs):
    ...
    if pretrained:
        ...
        elif os.path.exists("video_evaluate/DOVER/pretrained_weights"):
            model_dir = "video_evaluate/DOVER/pretrained_weights"
        else:
            raise ValueError("pretrained_weights folder not found")
        checkpoint = torch.hub.load_state_dict_from_url(url=url, model_dir=model_dir, map_location="cpu", check_hash=True)
